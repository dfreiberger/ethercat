...
pub mod ioctl {
    ...
    ioctl!(write      SLAVE_DICT_UPLOAD      with EC, 0x1f; ec_ioctl_slave_dict_upload_t);
    ioctl!(none       REQUEST                with EC, 0x20);
    ioctl!(none       CREATE_DOMAIN          with EC, 0x21);
    ioctl!(readwrite  CREATE_SLAVE_CONFIG    with EC, 0x22; ec_ioctl_config_t);
    ioctl!(write      SELECT_REF_CLOCK       with EC, 0x23; u32);
    ioctl!(read       ACTIVATE               with EC, 0x24; ec_ioctl_master_activate_t);
    ioctl!(none       DEACTIVATE             with EC, 0x25);
    ioctl!(arg        SEND                   with EC, 0x26);
    ioctl!(none       RECEIVE                with EC, 0x27);
    ioctl!(read       MASTER_STATE           with EC, 0x28; ec_master_state_t);
    ioctl!(readwrite  MASTER_LINK_STATE      with EC, 0x29; ec_ioctl_link_state_t);
    ioctl!(write      APP_TIME               with EC, 0x2a; ec_ioctl_app_time_t);
    ioctl!(none       SYNC_REF               with EC, 0x2b);
    ...
    ioctl!(none       REF_CLK_TIME_QUEUE     with EC, 0x62);
    ioctl!(read       REF_CLK_TIME           with EC, 0x63; u64);
